...
function joinUrl() {
  let urlParts = [].slice.call(arguments)
  return urlParts
    .reduce((ret, urlPart) =>
        ret.replace(/\/$/, '') +
        (ret ? '/' : '') +
...
function addToQuerystring(url, params) {
  return url + (~url.indexOf('?') ? '&' : '?') + qs.stringify(params)
}
...
function connect(req, res) {
  ...
  res.http_header('Location', config['authorize_url'])
...
function callback(req, res) {
  ...
  return config.client.authorizationCode.getToken({
